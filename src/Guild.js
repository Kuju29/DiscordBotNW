...
class Guild {
      ...
      if (warChannel) {
        if (warChannel.type === ChannelType.GuildVoice) {
          if (
            ...
            interaction.reply(
              `Missing permissions to speak and/or join in ${this.warChannel}`
            );
            return;
          }
          ...
          interaction.reply(
            "Channel " + this.warChannel + " is not a voice channel"
          );
        }
      } else {
  ...
  startWar(msg, time) {
    ...
    if (!validateTimeArgsMinuteSeconds(timeArgs)) {
      msg.reply(
        time + " is not a valid time. Type /help for a list of commands"
      );
      return;
    }
    const current = new Date();
    const warStartMillis = new Date(
      current.getFullYear(),
      current.getMonth(),
      current.getDate(),
      current.getHours(),
      current.getMinutes() + parseInt(timeArgs[0]),
      current.getSeconds() + parseInt(timeArgs[1])
    ).getTime();
    if (this.checkForCollidingTimers(warStartMillis)) {
      msg.reply(
        time +
          " collides with a different war. !leaveWar the current war or use /list and /unschedule-war unwanted wars."
      );
      return;
    }
    const war = new War(
  ...
  scheduleWar(msg, time) {
    ...
    if (this.checkForCollidingTimers(warStartMillis)) {
      msg.reply(
        time +
          " collides with a different war. /leaveWar the curent war or use /list and /unschedule-war unwanted wars."
      );
      return;
    }
    const war = new War(
  ...
  checkForCollidingTimers(warStartMillis) {
    const collidingTimers =
      [...this.startTimerWarMap.keys()].filter(
        (time) =>
          Math.abs(time - warStartMillis) <
          1000 * 60 * 30 + this.preJoinTimer * 1000
      ).length != 0;
    const currentWarColliding = this.activeWar
      ? Math.abs(this.activeWar.warStart - warStartMillis) <
        1000 * 60 * 30 + this.preJoinTimer * 1000
      : false;
  ...
  handleUnscheduleWar(interaction) {
    ...
    Array.from(this.startTimerWarMap.keys()).forEach((timer) => {
      timeStringToTimestamp.set(
        millisToCETString(timer + this.timeZone),
        timer
      );
    });
  ...
  handleList(interaction) {
    ...
    interaction.reply(
      "Current scheduled Wars: " + (res.length ? res.join(", ") : "none")
    );
  }
  ...
  handleGetSettings(interaction) {
    this.getFirstChannelFromName(this.warChannel).then((warChannel) => {
      const embed = new EmbedBuilder()
        .setDescription("All current settings")
        .setTitle("Settings")
        .setColor(0xaaaaff)
        .addFields([
          {
          ...
          {
            name: `\`Timezone\``,
            value: `Time matching your local time ${millisToCETString(
              new Date().getTime() + this.timeZone
            )}`,
          },
        ]);
  ...
  setPreJoinTimer(interaction) {
    ...
    guildService.save(this);
    interaction.reply(
      `Pre Join Timer has been changed to ${this.preJoinTimer} seconds`
    );
  }
  ...
  setFirstCallTimer(interaction) {
    ...
    guildService.save(this);
    interaction.reply(
      `First Call Timer has been changed to ${this.firstCallTimer} seconds`
    );
  }
  ...
  setCallRate(interaction) {
    ...
    if (!nums) {
      interaction.reply(
        input +
          " contains non numbers or is in the wrong format. Type /help for a list of commands"
      );
      return;
    }
  ...
  setTimezone(interaction) {
    ...
    if (interaction.options.data.length > 0) {
      time = interaction.options.data[0].value;
    } else {
    ...
    if (this.startTimerWarMap.size) {
      interaction.reply(
        "Can't change timezone while scheduling wars. Use /list and /unschedule-war all listed wars."
      );
      return;
    }
    ...
    guildService.save(this);
    interaction.reply(
      `Timezone has been set to match the current time of ${time}.`
    );
  }
...
function buildSettingsDescriptorMap() {
  ...
  [
    ...
    },
    {
      ...
      value: "300",
      description:
        "Time in seconds before the bot should join the war channel.",
    },
    {
      name: "first-call-timer",
      value: "600",
      description:
        "Time in seconds after the bot starts calling waves in war (to reduce spam for frequent early respawn waves).",
    },
    {
      name: "call-rate",
      value: "5 10 15",
    ...
    },
    {
      ...
      value: "19:30",
      description:
        "Set the bots time calcluations to match the current time. Either put in your local time or the time of the server you are playing on.",
    },
